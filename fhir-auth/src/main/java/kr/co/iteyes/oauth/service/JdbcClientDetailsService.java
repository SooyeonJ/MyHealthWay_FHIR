...
public class JdbcClientDetailsService implements ClientDetailsService, ClientRegistrationService {
    ...
    private static final String BASE_ENCRYPT_FIND_STATEMENT = "select client_id, " + " pls_decrypt_b64_id( client_secret, 101 ), resource_ids, scope, "
            + "authorized_grant_types, web_server_redirect_uri, authorities, access_token_validity, "
            + "refresh_token_validity, additional_information, autoapprove"
    ...
    private static final String DEFAULT_ENCRYPT_SELECT_STATEMENT = BASE_ENCRYPT_FIND_STATEMENT + " where client_id = ?";


    private static final String DEFAULT_INSERT_STATEMENT = "insert into oauth_client_details (" + CLIENT_FIELDS
            + ", client_id) values (?,pls_encrypt_b64_id(?,101),?,?,?,?,?,?,?,?,?)";

    private static final String Encrpyt_INSERT_STATEMENT = "insert into oauth_client_details (" + CLIENT_FIELDS
            + ", client_id) values (pls_encrypt_b64_id(?,101),?,?,?,?,?,?,?,?,?,?)";
    ...
    private static final String DEFAULT_SECRET_ENCRYPT = "SELECT pls_decrypt_b64_id( ? , 101 ) FROM dual;";
    ...
    private String insertEncrpytClientDetailsSql = Encrpyt_INSERT_STATEMENT;

    private String selectClientDetailsSql = DEFAULT_SELECT_STATEMENT;

    private String selectEncryptDetailSql = DEFAULT_ENCRYPT_SELECT_STATEMENT;
    ...
    public ClientDetails loadEncryptClientByClientId(String clientId) throws InvalidClientException, SQLException, UncategorizedSQLException {
        ClientDetails details;
        try {
            details = jdbcTemplate.queryForObject(selectEncryptDetailSql, new ClientDetailsRowMapper(), clientId);
        } catch (EmptyResultDataAccessException e) {
            log.error(e.getMessage(), e);
            throw new NoSuchClientException("No client with requested id");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            throw new RuntimeException("클라이언트를 찾을 수 없습니다.");
        }
        return details;
    }
    ...
    public void addEcryptClientDetails(ClientDetails clientDetails) throws ClientAlreadyExistsException {
        try {
            jdbcTemplate.update(insertEncrpytClientDetailsSql, getFields(clientDetails));
        } catch (DuplicateKeyException e) {
            throw new ClientAlreadyExistsException("Client already exists", e);
        }
    }
    ...
    public void setEncryptInsertClientDetailsSql(String insertEncrpytClientDetailsSql) {
        this.insertEncrpytClientDetailsSql = insertEncrpytClientDetailsSql;
    }
