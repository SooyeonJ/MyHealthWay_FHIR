...
public class FhirServerConfigCommon {
	...
	public FhirServerConfigCommon(AppProperties appProperties) {
		...
		ourLog.info("Server configured to " + (appProperties.getEnable_repository_validating_interceptor() ? "true" : "false") + " enable_repository_validating_interceptor"); // 2023-08-25 유효성 검증 여부 체크 추가

		// iteyes start
		ourLog.info("Server configured to " + (appProperties.getAllow_cascading_deletes() ? "allow" : "disable") + " allow_cascading_deletes");
		// iteyes end
	...
	public DaoConfig daoConfig(AppProperties appProperties) {
		...
		// iteyes custom start
//		daoConfig.setDeleteEnabled(false);
		daoConfig.setMatchUrlCacheEnabled(true);
		daoConfig.setEnforceReferenceTargetTypes(false);
		daoConfig.setAllowInlineMatchUrlReferences(false);
		daoConfig.setMassIngestionMode(true);
//		daoConfig.setTagStorageMode(DaoConfig.TagStorageModeEnum.INLINE);
//		daoConfig.setConcurrentBundleValidation(true);
		// iteyes custom end
