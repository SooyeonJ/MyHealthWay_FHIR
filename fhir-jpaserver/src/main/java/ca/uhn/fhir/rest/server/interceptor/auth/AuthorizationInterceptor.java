...
public class AuthorizationInterceptor implements IRuleApplier {
	...
	private ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum myDefaultPolicy = ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum.DENY;
	private Set<ca.uhn.fhir.rest.server.interceptor.auth.AuthorizationFlagsEnum> myFlags = Collections.emptySet();
	private IValidationSupport myValidationSupport;

	private ca.uhn.fhir.rest.server.interceptor.auth.IAuthorizationSearchParamMatcher myAuthorizationSearchParamMatcher;
	 ...
	 */
	public AuthorizationInterceptor(ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum theDefaultPolicy) {
		this();
	...
	private void applyRulesAndFailIfDeny(RestOperationTypeEnum theOperation, RequestDetails theRequestDetails, IBaseResource theInputResource, IIdType theInputResourceId,
		...
		if (decision.getDecision() == ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum.ALLOW) {
			return;
		}
	...
	public Verdict applyRulesAndReturnDecision(RestOperationTypeEnum theOperation, RequestDetails theRequestDetails, IBaseResource theInputResource, IIdType theInputResourceId,
		...
		Set<ca.uhn.fhir.rest.server.interceptor.auth.AuthorizationFlagsEnum> flags = getFlags();
//		ourLog.info("Applying {} rules to render an auth decision for operation {}, theInputResource type={}, theOutputResource type={} ", rules.size(), theOperation,
...
//			ourLog.info("Rule being applied - {}", nextRule);
			...
			if (verdict != null) {
//				ourLog.info("Rule {} returned decision {}", nextRule, verdict.getDecision());
				break;
			}
		}

		if (verdict == null) {
			ourLog.info("No rules returned a decision, applying default {}", myDefaultPolicy);
	 ...
	 */
	public void setAuthorizationSearchParamMatcher(@Nullable ca.uhn.fhir.rest.server.interceptor.auth.IAuthorizationSearchParamMatcher theAuthorizationSearchParamMatcher) {
	...
	/**
	 * The default policy if no rules have been found to apply. Default value for this setting is {@link ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum#DENY}
	 */
	public ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum getDefaultPolicy() {
	 ...
	 */
	public AuthorizationInterceptor setDefaultPolicy(ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum theDefaultPolicy) {
	 ...
	 */
	public Set<ca.uhn.fhir.rest.server.interceptor.auth.AuthorizationFlagsEnum> getFlags() {
	 ...
	 * @see #setFlags(ca.uhn.fhir.rest.server.interceptor.auth.AuthorizationFlagsEnum...)
	 */
	public AuthorizationInterceptor setFlags(Collection<ca.uhn.fhir.rest.server.interceptor.auth.AuthorizationFlagsEnum> theFlags) {
	...
	/**
	 * Handle an access control verdict of {@link ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum#DENY}.
	 * <p>
	...
	public static class Verdict {
		...
		private final ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum myDecision;

		public Verdict(ca.uhn.fhir.rest.server.interceptor.auth.PolicyEnum theDecision, IAuthRule theDecidingRule) {
