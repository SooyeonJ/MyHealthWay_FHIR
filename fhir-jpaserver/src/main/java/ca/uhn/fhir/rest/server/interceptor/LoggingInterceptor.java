...
public class LoggingInterceptor {
	...
	/**
	 * ResponseBody logging Interceptor
	 * <p>
	 * SERVER_OUTGOING_RESPONSE : response 준비 시점
	 *
	 * @param theRequestDetails
	 * @param iBaseResource     fhir resource
	 * @author smlee
	 */
	@Hook(value = Pointcut.SERVER_OUTGOING_RESPONSE, order = 1)
	public boolean MhwResponseLogging(RequestDetails theRequestDetails, IBaseResource iBaseResource) {

		// ~/fhir/metadata responsebody 로그 출력 방지
		if (!iBaseResource.getClass().getName().equals("org.hl7.fhir.r4.model.CapabilityStatement") ||
			!theRequestDetails.getCompleteUrl().endsWith("/metadata")) {
			String responseBody = getResponseBody(theRequestDetails, iBaseResource);
			// ResponseBody logging
			// fhir resource가 포함되어 logging 되므로 주의
//			ourLog.info("response Body ::: {}");
	...
	/**
	 * parse, encode Response Body
	 *
	 * @param theRequestDetails
	 * @param theResource       fhir resource
	 * @author smlee
	 */
	private String getResponseBody(RequestDetails theRequestDetails, IBaseResource theResource) {
		String encoded;
	...
	public boolean handleException(RequestDetails theRequestDetails, BaseServerResponseException theException, HttpServletRequest theServletRequest, HttpServletResponse theServletResponse) throws Exception {
		if (myLogExceptions) {
			...
			myLogger.info("SERVER_HANDLE_EXCEPTION : " + line);
		}
		return true;
	}

	/**
	 * SERVER_PROCESSING_COMPLETED :: 서버로 들어오는 모든 request에 대해 성공여부 관계없이 항상 로그를 찍음
	 * @see Pointcut
	 */
